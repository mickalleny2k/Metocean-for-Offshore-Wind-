...
wind <- wind %>%
  mutate(height_bin = cut(wspeed, breaks=seq(0,16, by = 2),include.lowest= TRUE))

#Scatter diagram for incident wave conditions and kernel density
#colour palette
cols <- rev(rainbow(20)[-c(3,6,8,10,12,13,14,16,17,18,19,20)])

plot_kernel <- ggplot(wind,aes(x=wdir,y=wspeed)) +xlab("Wind Direction (dir_10m) (degrees)") +
  ylab("Wind speed (Speed_10m) (m/s)") +
  ...
  ggtitle("Speed (m/s) vs Direction (degrees)") +
  theme_linedraw()+
...
ggsave(plot_kernel, file="output/plots/wind_spped_dir_kernel_density.jpeg", dpi=600, height=5, width=7, units=c("in"))